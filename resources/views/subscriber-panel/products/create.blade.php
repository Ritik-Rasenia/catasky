                    ...
                    <div class="card-body p-4 pt-0">
                        <div class="row g-3">
                            ...
                            <div class="col-md-12">
                                ...
                                <input type="text" name="sku" class="form-control rounded-3" placeholder="Unique SKU / Part code" value="{{ old('sku') }}">
                            </div>
                            <div class="col-12">
                ...
                <div class="card border-0  rounded-4 mb-4">
                    ...
                    <div class="card-body p-4 pt-0">
                        ...
                        <div>
                            ...
                            <input type="text" name="tags" class="form-control rounded-3" placeholder="electrical, switches, indoor" value="{{ old('tags') }}">
                        </div>
                    </div>
                </div>
            ...
            <div class="col-lg-4">
                ...
                <div class="card border-0  rounded-4 mb-4">
                    ...
                    <div class="card-body px-4 pb-4 pt-0">
                        ...
                        <div class="mb-3 d-none" id="product-type-group">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" name="mrp" class="form-control rounded-3" placeholder="0.00" step="0.01" value="{{ old('mrp') }}">
                        </div>
                        <div class="mb-3">
                            <label class="form-label fw-semibold small text-muted text-uppercase">Offer Price (₹)</label>
                            <input type="number" name="offer_price" class="form-control rounded-3" placeholder="0.00" step="0.01" value="{{ old('offer_price') }}">
                        </div>
                    ...
                    <div class="card-body px-4 pb-4 pt-0">
                        <div class="mb-4">
                            ...
                            <div class="p-3 border border-dashed rounded-4 text-center bg-light">
                                <input type="file" name="thumbnail" class="form-control form-control-sm" accept="image/*" id="thumbnail-input" onchange="previewThumbnail(event)" required>
                            </div>
                            <div id="thumbnail-preview" class="mt-2 text-center" style="display:none;">
                                <img id="thumb-img" src="" alt="Preview" style="max-height:120px;border-radius:10px;object-fit:cover;border:1px solid #E2E8F0;" class="p-1 bg-white ">
                            </div>
                        </div>
                        <div>
                ...
                <div class="card border-0  rounded-4 mb-4">
                    ...
                    <div class="card-body px-4 pb-4 pt-0">
                        ...
                        <div class="form-check form-switch mt-2">
                            <input class="form-check-input" type="checkbox" name="featured" id="featuredCheck" {{ old('featured') ? 'checked' : '' }}>
                ...
                <div class="card border-0  rounded-4 sticky-top" style="top: 100px; z-index: 1;">
                    <div class="card-body p-4">
                        <button type="submit" name="status_submit" value="active" class="btn btn-primary w-100 rounded-pill py-2 mb-2 shadow fw-bold">
                            <i class="bi bi-check-circle me-2"></i>Publish Product
                        </button>
                        <button type="submit" name="status_submit" value="draft" class="btn btn-white border w-100 rounded-pill py-2 mb-2 fw-semibold">
                            Save as Draft
                        </button>
    ...
    $(document).ready(function() {
        ...
        function getAttributeInputHtml(attr) {
            ...
            switch(attr.type) {
                case 'text':
                case 'url':
                    inputHtml = `<input type="${attr.type === 'url' ? 'url' : 'text'}" name="attributes[${attr.id}]" class="form-control rounded-3" placeholder="${attr.placeholder || ''}" value="${attr.default_value || ''}" ${requiredAttr}>`;
                    break;
                case 'number':
                    inputHtml = `<input type="number" name="attributes[${attr.id}]" class="form-control rounded-3" placeholder="${attr.placeholder || '0'}" value="${attr.default_value || ''}" ${requiredAttr}>`;
                    break;
                case 'decimal':
                    inputHtml = `<input type="number" name="attributes[${attr.id}]" class="form-control rounded-3" step="any" placeholder="${attr.placeholder || '0.00'}" value="${attr.default_value || ''}" ${requiredAttr}>`;
                    break;
                case 'date':
                    inputHtml = `<input type="date" name="attributes[${attr.id}]" class="form-control rounded-3" value="${attr.default_value || ''}" ${requiredAttr}>`;
                    break;
                case 'color':
                    ...
                    break;
                case 'textarea':
                    inputHtml = `<textarea name="attributes[${attr.id}]" class="form-control rounded-3" rows="3" placeholder="${attr.placeholder || ''}" ${requiredAttr}>${attr.default_value || ''}</textarea>`;
                    break;
                case 'rich_text':
                    inputHtml = `
                        <div class="rich-text-editor-container" style="border:1.5px solid #E2E8F0; border-radius:10px; overflow:hidden;">
                            ...
                            <div id="editor-body-${attr.id}" contenteditable="true" class="p-3 bg-white" style="min-height:120px; outline:none; font-size:0.875rem;" oninput="updateRichTextValue('${attr.id}')">${attr.default_value || ''}</div>
                            <input type="hidden" name="attributes[${attr.id}]" id="editor-hidden-${attr.id}" value="${attr.default_value || ''}" ${requiredAttr}>
                        </div>
                    `;
                    break;
                case 'boolean':
                ...
                case 'image':
                case 'file':
                    inputHtml = `<input type="${attr.type}" name="attributes[${attr.id}]" class="form-control rounded-3" accept="${attr.type === 'image' ? 'image/*' : '*'}" ${requiredAttr}>`;
                    break;
            }
            ...
            $.ajax({
                ...
                success: function(res) {
                    ...
                    if (res.success) {
                        ...
                        // Prepend/append to PIM form section
