                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-8">
                                ...
                                <input type="text" name="name" class="form-control @error('name') is-invalid @enderror"
                                       placeholder="e.g. Industrial Grade Patch Panel" value="{{ old('name') }}" required>
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="text" name="sku" class="form-control @error('sku') is-invalid @enderror"
                                       placeholder="e.g. SKU-001" value="{{ old('sku') }}" required>
                            ...
                            <div class="col-12">
                                ...
                                <textarea name="short_description" class="form-control" rows="2"
                                          placeholder="A brief overview shown in catalogue listings">{{ old('short_description') }}</textarea>
                            </div>
                            {{-- Full Description --}}
                            <div class="col-12">
                                <label class="form-label">Full Description</label>
                                <textarea name="full_description" class="form-control editor" rows="5">{{ old('full_description') }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    function recalcDiscount() {
        ...
        if (mrp > 0 && op > 0 && mrp > op) {
            const pct = Math.round(((mrp - op) / mrp) * 100);
    ...
    $(document).ready(function() {
        ...
        $('#btn-save-new-brand').on('click', function() {
            ...
            $.ajax({
                ...
                error: function(xhr) {
                    $('#btn-save-new-brand').prop('disabled', false).text('Add');
                    $('#new-brand-error').removeClass('d-none').text(xhr.responseJSON?.errors?.name?.[0] || xhr.responseJSON?.message || 'Failed to create brand.');
                }
            });
        });
        ...
        categorySelect.on('change', function() {
            ...
            if (!catIds.length) {
                ...
                container.empty(); attrCard.addClass('d-none');
                return;
            }
        ...
        function buildAttrInput(attr) {
            ...
            switch(attr.type) {
                case 'text': case 'url':
                    inp = `<input type="${attr.type==='url'?'url':'text'}" name="attributes[${attr.id}]" class="form-control" placeholder="${attr.placeholder||''}" value="${attr.default_value||''}" ${reqAttr}>`;
                    break;
                case 'number':
                    inp = `<input type="number" name="attributes[${attr.id}]" class="form-control" placeholder="${attr.placeholder||'0'}" value="${attr.default_value||''}" ${reqAttr}>`;
                    break;
                case 'decimal':
                    inp = `<input type="number" name="attributes[${attr.id}]" class="form-control" step="any" placeholder="0.00" value="${attr.default_value||''}" ${reqAttr}>`;
                    break;
                case 'date':
                    inp = `<input type="date" name="attributes[${attr.id}]" class="form-control" value="${attr.default_value||''}" ${reqAttr}>`;
                    break;
                case 'textarea':
                    inp = `<textarea name="attributes[${attr.id}]" class="form-control" rows="3" placeholder="${attr.placeholder||''}" ${reqAttr}>${attr.default_value||''}</textarea>`;
                    break;
                case 'boolean':
                    inp = `<div class="form-check form-switch mt-2"><input type="checkbox" name="attributes[${attr.id}]" value="1" class="form-check-input" id="sw-${attr.id}" style="width:2.5em;height:1.25em;" ${attr.default_value=='1'||attr.default_value=='yes'?'checked':''}><label class="form-check-label small text-secondary ms-2" for="sw-${attr.id}" style="text-transform:none;letter-spacing:0;">Yes</label></div>`;
                    break;
                case 'select':
                    let sopts = `<option value="">-- Select --</option>`;
                    (attr.options||[]).forEach(o => { sopts += `<option value="${o.value}" ${o.is_default?'selected':''}>${o.label}</option>`; });
                ...
                case 'checkbox':
                    ...
                    (attr.options||[]).forEach(o => {
                        copts += `<label style="display:flex;align-items:center;gap:6px;cursor:pointer;color:#475569;font-size:.875rem;text-transform:none;letter-spacing:0;"><input type="checkbox" name="attributes[${attr.id}][]" value="${o.value}" ${o.is_default?'checked':''} style="accent-color:#4F46E5;width:16px;height:16px;">${o.label}</label>`;
                    });
                    copts += '</div>';
                    inp = copts;
                    break;
                default:
                    inp = `<input type="text" name="attributes[${attr.id}]" class="form-control" placeholder="${attr.placeholder||''}" value="${attr.default_value||''}" ${reqAttr}>`;
            }
            return `<div class="${col}"><div class="mb-2"><label class="form-label">${attr.name}${unit}${req}</label>${inp}</div></div>`;
        }

        // On load if category already selected
        if (categorySelect.val() && categorySelect.val().length > 0) {
            const oldSubIds = {!! json_encode(old('subcategory_id', [])) !!};
            const catIds = categorySelect.val();
            subcategorySelect.html('<option>Loading...</option>').trigger('change.select2');
            $.get('{{ route("subscriber.get-subcategories") }}', { category_id: catIds }, function(data) {
                let opts = '<option value="new_subcategory">＋ Add New Subcategory...</option>';
                data.forEach(s => {
                    const sel = oldSubIds.includes(String(s.id)) || oldSubIds.includes(Number(s.id)) ? 'selected' : '';
                    opts += `<option value="${s.id}" ${sel}>${s.name}</option>`;
                });
                subcategorySelect.html(opts).trigger('change.select2');
                const selSubcats = subcategorySelect.val() || [];
                if (selSubcats.length) loadAttributes(selSubcats);
            }).fail(function() {
                subcategorySelect.html('<option value="new_subcategory">＋ Add New Subcategory...</option>').trigger('change.select2');
            });
        } else {
