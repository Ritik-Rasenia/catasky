...
<head>
    ...
    @php
        ...
        $metaDescription = $globalSetting->site_description ?? 'Catasky catalogue subscriber panel.';
        ...
        $profile = $currentUser?->subscriberProfile;
        $primaryColor = $profile?->primary_color ?? $globalSetting->primary_color ?? '#4F46E5';
        $secondaryColor = $profile?->secondary_color ?? $globalSetting->secondary_color ?? '#7C3AED';
    @endphp
    <title>{{ $adminTitle ? $adminTitle . ' | ' : '' }}{{ $siteTitle }} Workspace</title>
...
<body data-sidebar-collapsed="false">
    @include('subscriber-panel.layouts.sidebar')

    <div id="main-content">
        @include('subscriber-panel.layouts.navbar')
                ...
                <div class="content-breadcrumb">
                    ...
                    @else
                        @php
                            ...
                            $url = '';
                        @endphp
                        <a href="{{ route('dashboard') }}"><i class="bi bi-house-door-fill me-1.5" style="font-size: 14px;"></i> Dashboard</a>
                        @foreach($segments as $segment)
                            @if($segment !== 'dashboard' && $segment !== 'subscriber' && $segment !== 'subscriber-panel')
                                @php
                                    $url .= '/' . $segment;
                                ...
                                @endphp
                                <i class="bi bi-chevron-right text-muted mx-2" style="font-size: 9px;"></i>
                                @if(!$loop->last)
        ...
        @include('subscriber-panel.layouts.footer')
    </div>
