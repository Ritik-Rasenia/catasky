        ...
        <div class="card border-0 shadow-sm h-100" style="border-radius: 16px; background: #ffffff;">
            ...
            <div class="card-body p-4">
                ...
                @else
                    ...
                    <form action="{{ route('subscriber.domain.store') }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <span class="text-muted d-block mt-2 small" style="font-size: 0.75rem;">Examples: ritikstore.com, www.ritikstore.com, catalog.ritikstore.com</span>
                        </div>
                ...
                <div class="mt-4 pt-4 border-top" style="border-color: #f1f5f9 !important;">
                    ...
                    @if($dom->status === 'Pending DNS Setup')
                    ...
                    @elseif($dom->status === 'Rejected')
                        <div class="p-3 rounded-3 mt-3 text-danger border border-danger border-opacity-20 d-flex gap-2" style="background-color: #fef2f2; font-size:0.78rem; line-height:1.5;">
                            ...
                            <span>This custom domain request was rejected. Reason: {{ $dom->rejection_reason ?: 'No reason provided.' }}</span>
                        </div>
                    ...
                    @else
                        <div class="p-3 rounded-3 mt-3 text-primary border border-primary border-opacity-20 d-flex gap-2" style="background-color: #eff6ff; font-size:0.78rem; line-height:1.5;">
                            <i class="bi bi-hourglass-split text-primary fs-6"></i>
                            <span>Domain DNS verified & SSL configured. Pending admin approval.</span>
                        </div>
                    @endif
                </div>
                @endif
        ...
        <div class="card border-0 shadow-sm h-100" style="border-radius: 16px; background: #ffffff;">
            <div class="card-header bg-transparent border-0 pt-4 px-4 d-flex align-items-center justify-content-between">
                <div class="d-flex align-items-center">
                    ...
                    <h5 class="card-title fw-bold text-dark mb-0" style="font-size: 1.1rem;">Active Custom Domain</h5>
                </div>
                ...
                @else
                    ...
                    <div class="mb-4 d-flex align-items-center justify-content-between flex-wrap gap-3">
                        ...
                        <div class="d-flex align-items-center gap-2 flex-wrap">
                            <!-- Overall Domain Status badge -->
                            @if($dom->status === 'Active')
                                <span class="premium-badge badge-active"><i class="bi bi-check-circle-fill"></i> Active</span>
                            @elseif($dom->status === 'Rejected')
                                <span class="premium-badge badge-suspended"><i class="bi bi-x-circle-fill"></i> Rejected</span>
                            @elseif($dom->status === 'Pending DNS Setup')
                                <span class="premium-badge badge-pending"><i class="bi bi-gear-fill"></i> Pending DNS Setup</span>
                            @elseif($dom->status === 'Pending Verification')
                                <span class="premium-badge badge-pending"><i class="bi bi-hourglass-split"></i> Pending Verification</span>
                            @elseif($dom->status === 'DNS Verified' || $dom->status === 'Verified')
                                <span class="premium-badge badge-active" style="background-color: rgba(59, 130, 246, 0.1); color: #3b82f6; border-color: rgba(59, 130, 246, 0.2);"><i class="bi bi-shield-check"></i> DNS Verified</span>
                            @elseif($dom->status === 'SSL Generating')
                                <span class="premium-badge badge-pending"><i class="bi bi-arrow-repeat spin"></i> SSL Generating</span>
                            @else
                                <span class="premium-badge badge-pending"><i class="bi bi-hourglass-split"></i> {{ $dom->status }}</span>
                            @endif

                            <!-- SSL Status badge -->
                            @if($dom->ssl_status === 'SSL Active')
                                <span class="premium-badge badge-active"><i class="bi bi-lock-fill"></i> SSL Active</span>
                            @elseif($dom->ssl_status === 'SSL Generating')
                                <span class="premium-badge badge-pending"><i class="bi bi-arrow-repeat spin"></i> SSL Generating</span>
                            @elseif($dom->ssl_status === 'SSL Failed')
                                <span class="premium-badge badge-suspended"><i class="bi bi-shield-slash-fill"></i> SSL Failed</span>
                            @else
                                <span class="premium-badge badge-pending"><i class="bi bi-shield-slash"></i> SSL Pending</span>
                            @endif

                            <!-- Admin Approval badge -->
                            @if($dom->admin_approved)
                                <span class="premium-badge badge-active"><i class="bi bi-person-check-fill"></i> Approved</span>
                            @elseif($dom->status === 'Rejected')
                                <span class="premium-badge badge-suspended"><i class="bi bi-person-x-fill"></i> Rejected</span>
                            @else
                                <span class="premium-badge badge-pending"><i class="bi bi-person-fill-gear"></i> Pending Approval</span>
                            @endif
                        </div>
                    </div>

                    {{-- Rejection Reason Alert if Rejected --}}
                    @if($dom->status === 'Rejected')
                        <div class="alert alert-danger border-0 d-flex gap-2 mb-4" role="alert" style="border-radius:12px; background:#FEE2E2; color:#991B1B;">
                            <i class="bi bi-x-octagon-fill fs-5"></i>
                            <div>
                                <strong class="d-block mb-1">Domain Request Rejected</strong>
                                <span>{{ $dom->rejection_reason ?: 'The domain details or DNS mapping were invalid.' }}</span>
                            </div>
                        </div>
                    @endif
                                ...
                                <tbody>
                                    ...
                                    <tr style="border-bottom: 1px solid #f1f5f9;">
                                        ...
                                        <td class="py-3 font-monospace text-secondary fw-semibold">
                                            <span>@</span>
                                            <button class="copy-btn ms-1" id="copy-txt-key" onclick="copyValue('@', 'copy-txt-key')" title="Copy Host Key"><i class="bi bi-clipboard"></i></button>
                                        </td>
                                        ...
                                        <td class="pe-3 py-3 text-end">
                                            @if($dom->dns_txt_verified)
                                                <span class="text-success fw-bold"><i class="bi bi-check-circle-fill"></i> verified</span>
                                            @else
                                                <span class="text-warning small"><i class="bi bi-hourglass-split"></i> pending</span>
                                            @endif
                                        </td>
                                    </tr>
                                    ...
                                    <tr style="border-bottom: 1px solid #f1f5f9;">
                                        ...
                                        <td class="py-3 font-monospace text-secondary fw-semibold">
                                            <span>{{ $cnameHost }}</span>
                                            <button class="copy-btn ms-1" id="copy-cname-key" onclick="copyValue('{{ $cnameHost }}', 'copy-cname-key')" title="Copy Host"><i class="bi bi-clipboard"></i></button>
                                        </td>
                                        <td class="py-3 font-monospace text-secondary">
                                            <span>domain.catasky.com</span>
                                            <button class="copy-btn ms-1" id="copy-cname-val" onclick="copyValue('domain.catasky.com', 'copy-cname-val')" title="Copy Destination"><i class="bi bi-clipboard"></i></button>
                                        </td>
                                        <td class="pe-3 py-3 text-end">
                                            @if($dom->dns_cname_verified)
                                                <span class="text-success fw-bold"><i class="bi bi-check-circle-fill"></i> verified</span>
                                            @else
                                                <span class="text-warning small"><i class="bi bi-hourglass-split"></i> pending</span>
                                            @endif
                            ...
                            @if($dom->status === 'Active')
                                <div class="p-3.5 rounded-3 text-success border border-success border-opacity-20 d-flex align-items-start gap-2.5" style="background-color: #f0fdf4; font-size: 0.85rem; line-height: 1.6;">
                                    ...
                                    <div>
                                        ...
                                        CNAME and IP routing are connected. SSL certification is Active ✅. Traffic is routing directly to your store storefront.
                                    </div>
                                </div>
                            @elseif($dom->status === 'Rejected')
                                <div class="p-3.5 rounded-3 text-danger border border-danger border-opacity-20 d-flex align-items-start gap-2.5" style="background-color: #fef2f2; font-size: 0.85rem; line-height: 1.6;">
                                    ...
                                    <div>
                                        <strong class="d-block mb-0.5" style="font-weight:700;">Domain Verification Rejected</strong>
                                        The domain request was rejected. Please review the reason above, update your settings, or contact support.
                                    </div>
                                </div>
                            @elseif($dom->status === 'DNS Verified' || $dom->status === 'Verified' || $dom->status === 'Pending Admin Review' || $dom->status === 'SSL Generating')
                                <div class="p-3.5 rounded-3 text-primary border border-primary border-opacity-20 d-flex align-items-start gap-2.5" style="background-color: #eff6ff; font-size: 0.85rem; line-height: 1.6;">
                                    <i class="bi bi-info-circle-fill fs-5 text-primary"></i>
                                    <div>
                                        <strong class="d-block mb-0.5" style="font-weight:700;">DNS Verified</strong>
                                        All DNS records are successfully verified. We are waiting for Super Admin approval before the routing goes live.
                                    </div>
                                </div>
                            @else
                    ...
                    {{-- Verification Audit Logs Section --}}
                    <div class="mt-5 pt-4 border-top" style="border-color: #f1f5f9 !important;">
                        <div class="d-flex align-items-center justify-content-between mb-3">
                            <h6 class="fw-bold text-dark mb-0" style="font-size: 0.95rem;"><i class="bi bi-journal-text text-primary me-2"></i>Verification Audit Logs</h6>
                            <button class="btn btn-sm btn-outline-secondary py-1 px-3 fw-semibold" type="button" data-bs-toggle="collapse" data-bs-target="#auditLogsCollapse" aria-expanded="true" aria-controls="auditLogsCollapse" style="border-radius: 8px; font-size: 0.75rem;">
                                Toggle Logs
                            </button>
                        </div>
                        <div class="collapse show" id="auditLogsCollapse">
                            <div class="table-responsive rounded-3 border" style="border-color: #f1f5f9 !important; background-color: #fafbfc; max-height: 250px; overflow-y: auto;">
                                <table class="table table-hover align-middle mb-0" style="font-size: 0.78rem;">
                                    <thead>
                                        <tr class="table-light text-secondary small fw-bold" style="background-color:#f8fafc; border-bottom: 1px solid #f1f5f9;">
                                            ...
                                            <th class="pe-3 py-2">MESSAGE</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @forelse($dom->logs as $log)
                                            <tr style="border-bottom: 1px solid #f1f5f9;">
                                                <td class="ps-3 py-2.5 font-monospace text-secondary" style="white-space: nowrap;">
                                                    {{ $log->created_at->format('Y-m-d H:i:s') }}
                                                </td>
                                                <td class="py-2.5 fw-bold text-dark text-uppercase" style="font-size:0.7rem;">
                                                    {{ str_replace('_', ' ', $log->action) }}
                                                </td>
                                                <td class="py-2.5">
                                                    @if($log->status === 'success')
                                                        <span class="badge bg-success bg-opacity-10 text-success border border-success border-opacity-20 px-2 py-0.5 fw-bold" style="font-size: 0.65rem; border-radius: 4px;">SUCCESS</span>
                                                    @elseif($log->status === 'failed')
                                                        <span class="badge bg-danger bg-opacity-10 text-danger border border-danger border-opacity-20 px-2 py-0.5 fw-bold" style="font-size: 0.65rem; border-radius: 4px;">FAILED</span>
                                                    @else
                                                        <span class="badge bg-secondary bg-opacity-10 text-secondary border border-secondary border-opacity-20 px-2 py-0.5 fw-bold" style="font-size: 0.65rem; border-radius: 4px;">INFO</span>
                                                    @endif
                                                </td>
                                                <td class="pe-3 py-2.5 text-secondary text-wrap" style="max-width: 250px;">
                                                    {{ $log->message }}
                                                </td>
                                            </tr>
                                        @empty
                                            <tr>
                                                <td colspan="4" class="text-center py-3 text-muted">
                                                    No verification audit logs found. Logs will populate during verification runs.
                                                </td>
                                            </tr>
                                        @endforelse
                                    </tbody>
    ...
    if (verifyBtn) {
        verifyBtn.addEventListener('click', function() {
            ...
            .then(data => {
                ...
                if (data.success) {
                    window.alertService.successAlert('DNS Verified', data.message).then(() => {
                        window.location.reload();
                    });
                } else {
                    window.alertService.errorAlert('Verification Failed', data.message || 'The DNS changes have not propagated yet. Please check your domain DNS settings and try again.');
