...
<style>
    ...
    .card-section-title { font-size: 1rem; font-weight: 700; color: #0F172A; margin: 0; }
    .card-section-sub { font-size: 0.8rem; color: #94A3B8; margin: 2px 0 0; }

    .form-label {
        font-size: 0.75rem; font-weight: 700; color: #64748B;
        margin-bottom: 5px; text-transform: uppercase; letter-spacing: 0.05em; display: block;
    }
    ...
    .form-control, .form-select {
        ...
        transition: border-color 0.2s, box-shadow 0.2s; background: #FAFBFC; width: 100%;
    }
    .form-control:focus, .form-select:focus {
        border-color: #4F46E5; box-shadow: 0 0 0 3px rgba(79,70,229,0.12); background: #fff; outline: none;
    }
    .form-control::placeholder { color: #CBD5E1; }

    .select2-container--bootstrap-5 .select2-selection {
        border: 1.5px solid #E2E8F0 !important; border-radius: 10px !important;
        background: #FAFBFC !important; min-height: 46px !important; padding: 4px 8px !important;
    }
    ...
    .select2-container--bootstrap-5 .select2-selection--multiple .select2-selection__choice {
        ...
        border-radius: 6px !important; padding: 2px 8px !important; font-weight: 600 !important; font-size: 0.8rem !important;
    }
    ...
    .discount-badge {
        display: none; position: absolute; top: -10px; right: 0; background: #10B981;
        color: #fff; font-size: 0.65rem; font-weight: 700; padding: 2px 8px; border-radius: 20px; white-space: nowrap;
    }
    ...
    .btn-premium {
        ...
        font-weight: 700; border: none; border-radius: 10px; padding: 12px;
...
<div class="container-fluid py-2">
    <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data" id="product-form" novalidate>
        @csrf
        @method('PUT')
        <div class="row g-4">
            ...
            <div class="col-xl-8 col-lg-7">
                    ...
                    <div class="card-body p-4">
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" name="part_code" class="form-control" placeholder="e.g. PC-001"
                                       value="{{ old('part_code', $product->part_code) }}">
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">Part Number</label>
                                <input type="text" name="part_number" class="form-control" placeholder="Manufacturer Part Number"
                                       value="{{ old('part_number', $product->part_number) }}">
                            </div>
                            <div class="col-12">
                                <label class="form-label">Short Description</label>
                                <textarea name="short_description" class="form-control" rows="2"
                                          placeholder="Brief overview shown in catalogue listings">{{ old('short_description', $product->short_description) }}</textarea>
                            </div>
                            <div class="col-12">
                                <label class="form-label">Full Description</label>
                                <textarea name="additional_info" class="form-control editor" rows="5">{{ old('additional_info', $product->additional_info) }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card-body p-4">
                        <div class="row g-3">
                            <div class="col-12">
                                ...
                                @php $selectedBrands = old('brand_id', is_array($product->brand_id) ? $product->brand_id : [$product->brand_id]); @endphp
                                <select name="brand_id[]" id="brand-select" class="form-select search-select" multiple>
                                    ...
                                    @foreach($brands as $brand)
                                        <option value="{{ $brand->id }}" {{ is_array($selectedBrands) && in_array($brand->id, $selectedBrands) ? 'selected' : '' }}>{{ $brand->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6">
                                ...
                                @php $selectedCats = old('category_id', is_array($product->category_id) ? $product->category_id : [$product->category_id]); @endphp
                                <select name="category_id[]" id="category-select" class="form-select search-select" multiple>
                                    <option value="new_category">＋ Add New Category...</option>
                                    @foreach($categories as $category)
                                        <option value="{{ $category->id }}" {{ is_array($selectedCats) && in_array($category->id, $selectedCats) ? 'selected' : '' }}>{{ $category->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6">
                                ...
                                @php $selectedSubs = old('subcategory_id', is_array($product->subcategory_id) ? $product->subcategory_id : [$product->subcategory_id]); @endphp
                                <select name="subcategory_id[]" id="subcategory-select" class="form-select search-select"
                                    ...
                                    @foreach($subcategories as $s)
                                        <option value="{{ $s->id }}" {{ is_array($selectedSubs) && in_array($s->id, $selectedSubs) ? 'selected' : '' }}>{{ $s->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-12">
                                ...
                                <input type="text" name="tags" class="form-control" placeholder="e.g. industrial, cable, patch"
                    ...
                    <div class="card-body p-4">
                        <div class="row g-3">
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="number" min="1" name="moq" class="form-control" placeholder="1"
                ...
                <div class="card form-section-card border-0 mb-4 d-none" id="dynamic-attributes-card">
                    ...
                    <div class="card-body p-4" id="dynamic-attributes-container">
                        <div class="text-muted small py-1"><i class="bi bi-info-circle me-1"></i>Select a subcategory to load specifications.</div>
                    </div>
                </div>
                    ...
                    <div class="card-body p-4">
                        <div class="mb-3">
                            ...
                            <div class="upload-dropzone mb-2" onclick="document.getElementById('thumbnail-input').click()">
                                ...
                                <p class="small text-muted mb-0 mt-1">Click to replace image</p>
                            ...
                            <div id="thumbnail-preview" class="text-center mt-2">
                                ...
                                @if($product->thumbnail)
                                    <div id="thumb-container">
                                        <img id="thumb-img" src="{{ $product->thumbnail_url }}" alt="Thumbnail"
                                             style="max-height:110px;border-radius:10px;object-fit:cover;border:1px solid #E2E8F0;" class="p-1 bg-white mb-2">
                                        <div>
                                ...
                                @else
                                    <div id="thumb-container" style="display:none;">
                                        <img id="thumb-img" src="" alt="Thumbnail"
                                             style="max-height:110px;border-radius:10px;object-fit:cover;border:1px solid #E2E8F0;" class="p-1 bg-white border mb-2">
                                        <div>
                                            <button type="button" class="btn btn-sm btn-outline-danger" onclick="clearThumbnail()">Remove Image</button>
                                        </div>
                                    </div>
                                ...
                                <div class="d-flex flex-wrap gap-2">
                                    ...
                                    <div class="position-relative img-wrap-{{ $img->id }}" style="height:70px;width:70px;">
                                        <img src="{{ asset('uploads/products/gallery/' . $img->image) }}"
                                             style="height:70px;width:70px;border-radius:8px;object-fit:cover;border:1px solid #E2E8F0;">
                                        <button type="button" data-delete-url="{{ route('admin.product-images.destroy', $img->id) }}"
                                                onclick="deleteProductImage(this, {{ $img->id }})"
                                                style="position:absolute;top:4px;right:4px;background:rgba(239,68,68,0.9);color:white;border:none;border-radius:50%;width:20px;height:20px;display:flex;align-items:center;justify-content:center;font-size:.7rem;cursor:pointer;">
...
<script>
    ...
    function deleteProductImage(btn, imageId) {
        ...
        Swal.fire({ title: 'Delete this photo?', text: 'This action cannot be undone.', icon: 'warning',
            showCancelButton: true, confirmButtonColor: '#EF4444', cancelButtonColor: '#64748B', confirmButtonText: 'Yes, delete' })
        .then(result => {
            if (result.isConfirmed) {
                fetch(deleteUrl, { method: 'DELETE', headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}', 'Accept': 'application/json' } })
                .then(r => r.json()).then(data => {
                    if (data.success) { document.querySelector(`.img-wrap-${imageId}`).remove(); Swal.fire('Deleted!', 'Image removed.', 'success'); }
                    else { Swal.fire('Error!', 'Could not delete image.', 'error'); }
                });
            }
        });
    }
    ...
    $(document).ready(function() {
        ...
        subcategorySelect.on('change', function() { loadAttributes($(this).val()); });

        if (subcategorySelect.val() && subcategorySelect.val().length) {
            loadAttributes(subcategorySelect.val());
        }
        ...
        function buildAttrInput(attr) {
            ...
            let parsedVal = null;
            if (rawVal !== undefined && rawVal !== null) { try { parsedVal = JSON.parse(rawVal); } catch(e) { parsedVal = rawVal; } }
            let inp = '';
            switch(attr.type) {
                case 'text': case 'url':
                    inp=`<input type="${attr.type==='url'?'url':'text'}" name="attributes[${attr.id}]" class="form-control" placeholder="${attr.placeholder||''}" value="${rawVal!==undefined?rawVal:(attr.default_value||'')}" ${reqAttr}>`; break;
                case 'number':
                    inp=`<input type="number" name="attributes[${attr.id}]" class="form-control" placeholder="${attr.placeholder||'0'}" value="${rawVal!==undefined?rawVal:(attr.default_value||'')}" ${reqAttr}>`; break;
                case 'decimal':
                    inp=`<input type="number" name="attributes[${attr.id}]" class="form-control" step="any" placeholder="0.00" value="${rawVal!==undefined?rawVal:(attr.default_value||'')}" ${reqAttr}>`; break;
                case 'date':
                    inp=`<input type="date" name="attributes[${attr.id}]" class="form-control" value="${rawVal!==undefined?rawVal:(attr.default_value||'')}" ${reqAttr}>`; break;
                case 'textarea':
                    inp=`<textarea name="attributes[${attr.id}]" class="form-control" rows="3" placeholder="${attr.placeholder||''}" ${reqAttr}>${rawVal!==undefined?rawVal:(attr.default_value||'')}</textarea>`; break;
                case 'boolean':
                    const bc=rawVal=='1'||rawVal=='yes'||(rawVal===undefined&&(attr.default_value=='1'||attr.default_value=='yes'));
                    inp=`<div class="form-check form-switch mt-2"><input type="checkbox" name="attributes[${attr.id}]" value="1" class="form-check-input" id="sw-${attr.id}" style="width:2.5em;height:1.25em;" ${bc?'checked':''}><label class="form-check-label small text-secondary ms-2" for="sw-${attr.id}" style="text-transform:none;letter-spacing:0;">Yes</label></div>`; break;
                case 'select':
                    let sopts=`<option value="">-- Select --</option>`;
                    (attr.options||[]).forEach(o=>{const sel=(rawVal!==undefined&&String(o.value)===String(rawVal))||(rawVal===undefined&&o.is_default);sopts+=`<option value="${o.value}" ${sel?'selected':''}>${o.label}</option>`;});
                    inp=`<select name="attributes[${attr.id}]" class="form-select" ${reqAttr}>${sopts}</select>`; break;
                case 'multiselect':
                    let mopts='';
                    (attr.options||[]).forEach(o=>{const sel=Array.isArray(parsedVal)&&parsedVal.map(String).includes(String(o.value));mopts+=`<option value="${o.value}" ${sel?'selected':''}>${o.label}</option>`;});
                ...
                case 'checkbox':
                    ...
                    (attr.options||[]).forEach(o=>{const chk=Array.isArray(parsedVal)&&parsedVal.map(String).includes(String(o.value));copts+=`<label style="display:flex;align-items:center;gap:6px;cursor:pointer;color:#475569;font-size:.875rem;text-transform:none;letter-spacing:0;"><input type="checkbox" name="attributes[${attr.id}][]" value="${o.value}" ${chk?'checked':''} style="accent-color:#4F46E5;width:16px;height:16px;">${o.label}</label>`;});
                    copts+='</div>'; inp=copts; break;
                default:
                    inp=`<input type="text" name="attributes[${attr.id}]" class="form-control" placeholder="${attr.placeholder||''}" value="${rawVal!==undefined?rawVal:(attr.default_value||'')}" ${reqAttr}>`;
            }
